  ...
  <div class="c-take-question" v-loading="loading">
    <div class="c-take-question-content" v-if="currentQuestion">
      <div class="q-text-title">
        ...
        <div class="m-exam-op" >
          <Fav style="padding-top: 9px; padding-bottom: 9px" post-type="question" :post-id="id" />
          <el-button  v-if="isAdmin" type="warning" plain size="small" icon="el-icon-circle-close" @click="check('restore')">复审</el-button>
          <el-button  v-if="isAdmin" type="danger" plain size="small" icon="el-icon-delete" @click="check('delete')">删除</el-button>
        </div>
      </div>

      <div class="q-attr">
        <span class="q-attr-hint">{{ isMultiple ? '多选题' : '单选题' }}</span>
        <el-rate v-model="currentQuestion.hardStar" disabled text-color="#ff9900" score-template="{value}"></el-rate>
        ...
        <span class="q-attr-author">
          ...
          <span class="c-exam-attr-value">
            <el-tag size="medium" v-for="tag of JSON.parse(currentQuestion.tags)" :key="tag">{{ tag }}</el-tag>
          </span>
        </span>
      </div>
      ...
      <div class="c-exam-take-btn">
        <el-button type="success" @click="checkLogin" v-if="!isSubmitted && !loading && currentQuestion" :disabled="chosenOptions === null || JSON.stringify(chosenOptions) === '' || JSON.stringify(chosenOptions) === '[]'">提交</el-button>
      </div>
    ...
    <div class="m-exam-comment">
      <el-divider content-position="left"><i class="el-icon-chat-line-square"></i> 讨论</el-divider>
...
export default {
  ...
  methods: {
    ...
    async getRemoteQuestionInfo() {
      let getUrl = realUrl(__next, `api/question/${this.questionid}/no-answer`)
    ...
    submitQuestion(answer) {
      let postUrl = realUrl(__next, `api/question/user-exam-paper/q/${this.questionid}/answer`)
    ...
    isWrongAnswerClass(idx) {
      if (this.correctAnswer) {
        if (!this.correctAnswer.includes(idx) && this.chosenOptions !== null) {
